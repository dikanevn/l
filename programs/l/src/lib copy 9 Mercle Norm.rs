...
// Фиксированный адрес больше не используется – новый mint будет создан в ходе исполнения

// Корневой хеш меркл-дерева (полученный офф-чейн)
 // Представление меркл корня в формате [u8; 32]
...
pub struct MintOne<'info> {
    ...
    /// Новый mint, который инициализируется в рамках инструкции.
    #[account(
       init,
       payer = authority,
       mint::decimals = 0,
